...
import { google } from "googleapis"; // Google Drive API এর জন্য
...
const ROOT_FOLDER_ID = process.env.DRIVE_ROOT_FOLDER_ID || "1ok5OzWA5G0tzcSUP1hSlbuVt90gAcx3L";
let drive = null;
...
try {
  ...
  if (credentials.client_email && credentials.private_key) {
    ...
    console.warn("⚠️ GOOGLE_SERVICE_ACCOUNT_JSON not configured properly");
  }
} catch (e) {
  console.warn("⚠️ Google Drive auth failed:", e.message);
}

// ড্রাইভ থেকে ফাইল লিস্ট নিয়ে কনটেক্সট তৈরি
async function getDriveContext() {
  ...
  try {
    ...
    let context = "\n\n[GOOGLE DRIVE DATA — মেমোরি পোর্টাল ফাইলসমূহ]\n";
    res.data.files.forEach((f) => {
      const isImage = f.mimeType && f.mimeType.startsWith("image/");
      context += `- ${f.name}${isImage ? " [ছবি]" : ""} (ID: ${f.id})\n`;
    });
    return context;
  } catch (err) {
    console.warn("Drive context error:", err.message);
    return "";
  }
}
...
const GEMINI_KEYS = Array.from(
  new Set([...fromEnv("GEMINI_API_KEY", "GEMINI_API_KEY_2", "GEMINI_API_KEYS")])
);
...
const GROQ_KEYS = Array.from(
  new Set([...fromEnv("GROQ_API_KEY", "GROQ_API_KEY_2", "GROQ_API_KEYS")])
);
const OPENROUTER_KEYS = Array.from(
  new Set([...fromEnv("OPENROUTER_API_KEY", "OPENROUTER_API_KEY_2", "OPENROUTER_API_KEYS")])
);
const DEEPSEEK_KEYS = Array.from(
  new Set([...fromEnv("DEEPSEEK_API_KEY", "DEEPSEEK_API_KEY_2", "DEEPSEEK_API_KEYS")])
);
...
function makePool(keys, name) {
  ...
  return {
    ...
    next() {
      ...
      let best = keys[0],
        bestUntil = blocked.get(keys[0]) || 0;
      for (const k of keys) {
...
async function tryDeepseek(sys, contents) {
  ...
  try {
    ...
    console.warn("deepseek:", e.message);
    return null;
  }
}
...
async function tryOpenRouter(sys, contents) {
  ...
  try {
    ...
    console.warn("openrouter:", e.message);
    return null;
  }
}
...
function mount(prefix) {
  ...
  app.get(prefix + "/healthz", (_req, res) =>
    res.json({
      ...
      drive: !!drive,
      keys: {
        gemini: geminiPool.size,
        eleven: elevenPool.size,
        groq: groqPool.size,
        openrouter: orPool.size,
        deepseek: deepseekPool.size,
      },
    })
  );
  ...
  app.post(prefix + "/chat", async (req, res) => {
    try {
      ...
      let sys = (systemPrompt && String(systemPrompt).trim()) || DEFAULT_SYSTEM_PROMPT;

      // 🔥 Google Drive কনটেক্সট যোগ করা হচ্ছে
      const driveCtx = await getDriveContext();
      if (driveCtx) {
        sys +=
          driveCtx +
          "\n\nনির্দেশনা: উপরের ফাইল তালিকা থেকে প্রাসঙ্গিক ছবির আইডি [IMAGE: ID] ফরম্যাটে পাঠাও।";
      }
  ...
  app.post(prefix + "/analyze", async (req, res) => {
    try {
      const {
        prompt = "এই ফাইলটা বিশ্লেষণ করে বাংলায় সংক্ষেপে বল।",
        file,
        mime,
        systemPrompt,
      } = req.body || {};
