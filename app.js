...
(() => {
  ...
  let settings = {
    voice: "edge:bn-BD-NabanitaNeural", // default Microsoft Edge Bangla voice
    userName: "দাদা",
    prompt: "",          // empty = use server default (full Parisa memory prompt)
    logoUrl: "",
  };
  ...
  function renderMessages() {
    ...
    chat.messages.forEach((m, idx) => {
      ...
      if (m.text) {
        const parsed = (typeof marked !== "undefined") ? marked.parse(m.text) : m.text;
  ...
  async function speak(text) {
    ...
    try {
      ...
      // default: try edge then eleven then browser
  ...
  let micRec = null;          // for mic button (one-shot)
  let callRec = null;         // for continuous call
  ...
  async function populateVoices() {
    ...
    // 1) Microsoft Edge voices (preferred)
    let edge = [];
    try {
    ...
    // 2) ElevenLabs voices (if backend has keys)
    try {
  ...
  function startCall(type) {
    ...
    view.hidden = false;
    if (type === "video") startCamStream($("#videoCallVideo"));
    setCallStatus("শুনছি…");
    listenLoop();
  }
  function endCall() {
  ...
  function listenLoop() {
    ...
    callRec.onend = async () => {
      ...
      // when audio ends, restart listening
      ...
      // also a safety fallback for browser TTS
  ...
  // expose for inline onclick fallbacks (if any)
